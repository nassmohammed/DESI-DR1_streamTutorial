...
def get_colour_index_and_abs_mag(ebv, g_flux, r_flux, distances):
    ...
    # G_mag = g_mag - 5 * np.log10(distances) + 5 
...
def spline_project_model(theta, spline_x_points, pstream_spline_x_points, lsig_vgsr_spline_x_points, vgsr_min, vgsr_max, feh_min, feh_max, pmra_min, pmra_max, pmdec_min, pmdec_max, trunc_fit=True, feh_fit=True, param_labels=spline_param_labels,k=2):
    ...
    if isinstance(params['pstream_spline_points'], np.float64): 
        pstream = params['pstream_spline_points']
    elif len(params['pstream_spline_points']) > 3:
        pstream = (apply_spline(0,pstream_spline_x_points,params['pstream_spline_points'], k=3))
    elif len(params['pstream_spline_points']) == 1:
        pstream = params['pstream_spline_points']
    elif len(params['pstream_spline_points']) <= 3:
        pstream = (apply_spline(0,pstream_spline_x_points,params['pstream_spline_points'], k=len(params['pstream_spline_points'])-1))
...
# ==============================================================================
# UNIFIED LIKELIHOOD AND MEMBERSHIP PROBABILITY FUNCTIONS
# ==============================================================================

def unified_lnprob(theta, prior, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, phi1, pmcorr,
                   mode='basic', pm_model='2d', spline_config=None, fit_feh=True, truncated=True, 
                   const_lsigpm=False, assert_prior=False, **kwargs):
    """
    Unified likelihood function that handles all modeling modes.
    ...
    vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err : arrays
    ...
    mode : str, default 'basic'
        Modeling mode: 'basic', 'spline', or 'spline_1d'
    pm_model : str, default '2d'
        Proper motion model: '1d' or '2d'
    spline_config : dict, optional
        Configuration for spline modes with keys:
        - 'spline_x_points': main spline points
        - 'pstream_spline_x_points': stream probability spline points  
        - 'lsigv_spline_x_points': velocity dispersion spline points
        - 'k': spline degree (default 2)
        - 'reshape_arr_shape': array reshaping info
        - 'trunc_bounds': truncation bounds for 1D mode
    fit_feh : bool, default True
        Whether to fit metallicity
    truncated : bool, default True
        Whether to use truncated fits
    const_lsigpm : bool, default False
        Whether to use constant proper motion dispersions
    assert_prior : bool, default False
        Whether to assert prior bounds for debugging
    ...
    if mode == 'basic':
        return _basic_lnprob(theta, prior, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, 
                           pmdec, pmdec_err, phi1, pmcorr, pm_model, fit_feh, truncated, assert_prior)
    
    elif mode == 'spline':
        if spline_config is None:
            raise ValueError("spline_config must be provided for spline mode")
        return _spline_lnprob(theta, prior, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err,
                            pmdec, pmdec_err, phi1, pmcorr, spline_config, pm_model, 
                            fit_feh, truncated, const_lsigpm, assert_prior)
    
    elif mode == 'spline_1d':
        if spline_config is None:
            raise ValueError("spline_config must be provided for spline_1d mode")
        return _spline_1d_lnprob(theta, prior, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err,
                               pmdec, pmdec_err, phi1, spline_config, pm_model, fit_feh, 
                               truncated, **kwargs)
...
def unified_memprob(theta, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, phi1, pmcorr,
                    mode='basic', pm_model='2d', spline_config=None, fit_feh=True, truncated=True,
                    const_lsigpm=False, return_lik=False, **kwargs):
    """
    Unified membership probability function that handles all modeling modes.
    ...
    pm_model : str, default '2d'
        Proper motion model: '1d' or '2d'
    spline_config : dict, optional
        Configuration for spline modes (see unified_lnprob for details)
    fit_feh : bool, default True
        Whether to fit metallicity
    truncated : bool, default True
        Whether to use truncated fits
    const_lsigpm : bool, default False
        Whether to use constant proper motion dispersions
    return_lik : bool, default False
        Whether to return likelihood components
    
    Returns:
    --------
    array or tuple : Membership probabilities (and likelihood components if return_lik=True)
    """
    
    if mode == 'basic':
        return _basic_memprob(theta, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err,
                            pmdec, pmdec_err, phi1, pmcorr, pm_model, fit_feh, return_lik)
    
    elif mode == 'spline':
        if spline_config is None:
            raise ValueError("spline_config must be provided for spline mode")
        return _spline_memprob(theta, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err,
                             pmdec, pmdec_err, phi1, pmcorr, spline_config, pm_model,
                             fit_feh, const_lsigpm, return_lik)
    
    elif mode == 'spline_1d':
        if spline_config is None:
            raise ValueError("spline_config must be provided for spline_1d mode")
        return _spline_1d_memprob(theta, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err,
                                pmdec, pmdec_err, phi1, spline_config, pm_model, fit_feh,
                                truncated, **kwargs)
...
def _basic_lnprob(theta, prior, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, 
                  phi1, pmcorr, pm_model, fit_feh, truncated, assert_prior):
    """Basic likelihood function (equivalent to original lnprob)"""
    ...
    if pm_model == '2d':
        ...
        # 1D proper motion modeling
        lstream_pmra = stats.norm.logpdf(pmra, loc=quad_f(phi1,pmra1,pmra2,pmra3), scale=np.sqrt(pmra_err**2 + (10**lsigpmra)**2))
        lstream_pmdec = stats.norm.logpdf(pmdec, loc=quad_f(phi1,pmdec1,pmdec2,pmdec3), scale=np.sqrt(pmdec_err**2 + (10**lsigpmdec)**2))
        lstream_pm = lstream_pmra + lstream_pmdec
        
        lbg_pmra = stats.norm.logpdf(pmra, loc=bpmra, scale=np.sqrt(pmra_err**2 + (10**lsigbpmra)**2))
        lbg_pmdec = stats.norm.logpdf(pmdec, loc=bpmdec, scale=np.sqrt(pmdec_err**2 + (10**lsigbpmdec)**2))
        lbg_pm = lbg_pmra + lbg_pmdec
...
def _basic_memprob(theta, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, 
                   phi1, pmcorr, pm_model, fit_feh, return_lik):
    """Basic membership probability function (equivalent to original memprob)"""
...
def _spline_lnprob(theta, prior, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, 
                   phi1, pmcorr, spline_config, pm_model, fit_feh, truncated, const_lsigpm, assert_prior):
    """Spline-based likelihood function"""
    
    # Extract spline configuration
    spline_x_points = spline_config['spline_x_points']
    pstream_spline_x_points = spline_config.get('pstream_spline_x_points', spline_x_points)
    lsigv_spline_x_points = spline_config.get('lsigv_spline_x_points', spline_x_points)
    k = spline_config.get('k', 2)
    reshape_arr_shape = spline_config.get('reshape_arr_shape', None)
    
    if reshape_arr_shape is not None:
        reshaped_theta = reshape_arr(theta, reshape_arr_shape)
    else:
        reshaped_theta = theta
    
    # Extract parameters based on whether const_lsigpm is used
    if const_lsigpm:
        lpstream_spline_points, \
        vgsr_spline_points, lsigv_spline_points, \
        feh1, lsigfeh, \
    ...
    # Prior checking (simplified for spline case)
    for i in range(len(theta)):
        if len(prior) > i and len(prior[i]) >= 2:
            if (theta[i] < prior[i][0]) or (theta[i] > prior[i][1]):
                if assert_prior:
                    print(f"Parameter {i} out of bounds: {theta[i]} not in [{prior[i][0]}, {prior[i][1]}]")
                return -1e10
    
    # Apply splines to get values at phi1 locations
    lpstream_vals = apply_spline(phi1, pstream_spline_x_points, lpstream_spline_points, k)
    vgsr_vals = apply_spline(phi1, spline_x_points, vgsr_spline_points, k)
    lsigv_vals = apply_spline(phi1, lsigv_spline_x_points, lsigv_spline_points, k)
    pmra_vals = apply_spline(phi1, spline_x_points, pmra_spline_points, k)
    pmdec_vals = apply_spline(phi1, spline_x_points, pmdec_spline_points, k)
    
    pstream_vals = 10**lpstream_vals
    ...
    if not truncated:
        lstream_v = stats.norm.logpdf(vgsr, loc=vgsr_vals, scale=scale_stream_vgsr)
        lbg_v = stats.norm.logpdf(vgsr, loc=bv, scale=scale_bg_vgsr)
        ...
        min_trunc_vgsr, max_trunc_vgsr = np.min(vgsr), np.max(vgsr)
        lvgsr_cdf_dif = np.log(stats.norm.cdf(max_trunc_vgsr, loc=bv, scale=scale_bg_vgsr) - 
                              stats.norm.cdf(min_trunc_vgsr, loc=bv, scale=scale_bg_vgsr))
        lstream_v = stats.norm.logpdf(vgsr, loc=vgsr_vals, scale=scale_stream_vgsr)
        lbg_v = stats.norm.logpdf(vgsr, loc=bv, scale=scale_bg_vgsr) - lvgsr_cdf_dif
                
        if fit_feh:
            min_trunc_feh, max_trunc_feh = np.min(feh), np.max(feh)
            lfeh_cdf_dif = np.log(stats.norm.cdf(max_trunc_feh, loc=bfeh, scale=scale_bg_feh) - 
                                 stats.norm.cdf(min_trunc_feh, loc=bfeh, scale=scale_bg_feh))
            lstream_feh = stats.norm.logpdf(feh, loc=feh1, scale=scale_stream_feh)
            lbg_feh = stats.norm.logpdf(feh, loc=bfeh, scale=scale_bg_feh) - lfeh_cdf_dif
    
    # Proper motion likelihood
    if pm_model == '2d':
        lstream_pm = logpdf_2dnorm(pmra, pmdec, pmra_vals, pmdec_vals,
                                   pmra_err, pmdec_err, 10**lsigpmra, 10**lsigpmdec, pmcorr)
        lbg_pm = logpdf_2dnorm(pmra, pmdec, bpmra, bpmdec,
                               pmra_err, pmdec_err, 10**lsigbpmra, 10**lsigbpmdec, pmcorr)
    elif pm_model == '1d':
        lstream_pmra = stats.norm.logpdf(pmra, loc=pmra_vals, scale=np.sqrt(pmra_err**2 + (10**lsigpmra)**2))
        lstream_pmdec = stats.norm.logpdf(pmdec, loc=pmdec_vals, scale=np.sqrt(pmdec_err**2 + (10**lsigpmdec)**2))
        lstream_pm = lstream_pmra + lstream_pmdec
    ...
    # Combine components
    if fit_feh:
        lstream = np.log(pstream_vals) + lstream_v + lstream_feh + lstream_pm
        lbg = np.log(1-pstream_vals) + lbg_v + lbg_feh + lbg_pm
    else:
        lstream = np.log(pstream_vals) + lstream_v + lstream_pm
        lbg = np.log(1-pstream_vals) + lbg_v + lbg_pm
        
    ltot = np.logaddexp(lstream, lbg)
    return np.sum(ltot)


def _spline_memprob(theta, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, 
                    phi1, pmcorr, spline_config, pm_model, fit_feh, const_lsigpm, return_lik):
    """Spline-based membership probability function"""
    ...
    # Calculate likelihood components
    lstream_v = stats.norm.logpdf(vgsr, loc=vgsr_vals, scale=np.sqrt(vgsr_err**2+(10**lsigv_vals)**2))
    lbg_v = stats.norm.logpdf(vgsr, loc=bv, scale=np.sqrt(vgsr_err**2+(10**lsigbv)**2))
...
def _spline_1d_lnprob(theta, prior, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, 
                      phi1, spline_config, pm_model, fit_feh, truncated, **kwargs):
    """1D spline-based likelihood function"""
    
    # Extract spline configuration  
    spline_x_points = spline_config['spline_x_points']
    k = spline_config.get('k', 2)
    reshape_arr_shape = spline_config.get('reshape_arr_shape', None)
    trunc_bounds = spline_config.get('trunc_bounds', {
        'vgsr_trunc': [-np.inf, np.inf],
        'feh_trunc': [-np.inf, np.inf], 
        'pmra_trunc': [-np.inf, np.inf],
        'pmdec_trunc': [-np.inf, np.inf]
    })
    
    # Extract truncation bounds
    vgsr_trunc = trunc_bounds.get('vgsr_trunc', [-np.inf, np.inf])
    feh_trunc = trunc_bounds.get('feh_trunc', [-np.inf, np.inf])
    pmra_trunc = trunc_bounds.get('pmra_trunc', [-np.inf, np.inf])
    pmdec_trunc = trunc_bounds.get('pmdec_trunc', [-np.inf, np.inf])
    ...
    # Extract parameters (simplified for 1D case)
    lpstream_spline_points, \
    vgsr_spline_points, lsigv_spline_points, \
    feh1, lsigfeh, \
    pmra_spline_points, lsigpmra, \
    ...
    # Apply splines
    lpstream_vals = apply_spline(phi1, spline_x_points, lpstream_spline_points, k)
    vgsr_vals = apply_spline(phi1, spline_x_points, vgsr_spline_points, k)
    lsigv_vals = apply_spline(phi1, spline_x_points, lsigv_spline_points, k)
    pmra_vals = apply_spline(phi1, spline_x_points, pmra_spline_points, k)
    pmdec_vals = apply_spline(phi1, spline_x_points, pmdec_spline_points, k)
    
    # Stream likelihoods (1D modeling)
    lstream_v = stats.norm.logpdf(vgsr, loc=vgsr_vals, scale=np.sqrt(vgsr_err**2 + (10**lsigv_vals)**2))
    lstream_pmra = stats.norm.logpdf(pmra, loc=pmra_vals, scale=np.sqrt(pmra_err**2 + (10**lsigpmra)**2))
    lstream_pmdec = stats.norm.logpdf(pmdec, loc=pmdec_vals, scale=np.sqrt(pmdec_err**2 + (10**lsigpmdec)**2))
    ...
    # Background likelihoods with truncation if specified
    if truncated:
        # Truncated background distributions
        if vgsr_trunc[0] > -np.inf or vgsr_trunc[1] < np.inf:
            a_v = (vgsr_trunc[0] - bv) / (10**lsigbv)
            b_v = (vgsr_trunc[1] - bv) / (10**lsigbv)
            lbg_v = stats.truncnorm.logpdf(vgsr, a=a_v, b=b_v, loc=bv, scale=10**lsigbv)
        else:
            lbg_v = stats.norm.logpdf(vgsr, loc=bv, scale=10**lsigbv)
            
        if fit_feh and (feh_trunc[0] > -np.inf or feh_trunc[1] < np.inf):
            a_feh = (feh_trunc[0] - bfeh) / (10**lsigbfeh)
            b_feh = (feh_trunc[1] - bfeh) / (10**lsigbfeh)
            lbg_feh = stats.truncnorm.logpdf(feh, a=a_feh, b=b_feh, loc=bfeh, scale=10**lsigbfeh)
        elif fit_feh:
            lbg_feh = stats.norm.logpdf(feh, loc=bfeh, scale=10**lsigbfeh)
        else:
            lbg_feh = 0
            
        if pmra_trunc[0] > -np.inf or pmra_trunc[1] < np.inf:
            a_pmra = (pmra_trunc[0] - bpmra) / (10**lsigbpmra)
            b_pmra = (pmra_trunc[1] - bpmra) / (10**lsigbpmra)
            lbg_pmra = stats.truncnorm.logpdf(pmra, a=a_pmra, b=b_pmra, loc=bpmra, scale=10**lsigbpmra)
        else:
            lbg_pmra = stats.norm.logpdf(pmra, loc=bpmra, scale=10**lsigbpmra)
            
        if pmdec_trunc[0] > -np.inf or pmdec_trunc[1] < np.inf:
            a_pmdec = (pmdec_trunc[0] - bpmdec) / (10**lsigbpmdec)
            b_pmdec = (pmdec_trunc[1] - bpmdec) / (10**lsigbpmdec)
            lbg_pmdec = stats.truncnorm.logpdf(pmdec, a=a_pmdec, b=b_pmdec, loc=bpmdec, scale=10**lsigbpmdec)
        else:
            lbg_pmdec = stats.norm.logpdf(pmdec, loc=bpmdec, scale=10**lsigbpmdec)
    else:
        # Non-truncated background
        lbg_v = stats.norm.logpdf(vgsr, loc=bv, scale=10**lsigbv)
        lbg_pmra = stats.norm.logpdf(pmra, loc=bpmra, scale=10**lsigbpmra)
        lbg_pmdec = stats.norm.logpdf(pmdec, loc=bpmdec, scale=10**lsigbpmdec)
        if fit_feh:
    ...
    # Combine components
    if fit_feh:
        lstream = lpstream_vals + lstream_v + lstream_feh + lstream_pmra + lstream_pmdec
        lbg = np.log(1-(np.e**lpstream_vals)) + lbg_v + lbg_feh + lbg_pmra + lbg_pmdec
    else:
        lstream = lpstream_vals + lstream_v + lstream_pmra + lstream_pmdec
        lbg = np.log(1-(np.e**lpstream_vals)) + lbg_v + lbg_pmra + lbg_pmdec
        
    ltot = np.logaddexp(lstream, lbg)
    return np.sum(ltot)


def _spline_1d_memprob(theta, vgsr, vgsr_err, feh, feh_err, pmra, pmra_err, pmdec, pmdec_err, 
                       phi1, spline_config, pm_model, fit_feh, truncated, **kwargs):
    """1D spline-based membership probability function"""
    ...
    pstream_vals = np.e**lpstream_vals
    
    # Calculate likelihood components (1D modeling)
    lstream_v = stats.norm.logpdf(vgsr, loc=vgsr_vals, scale=np.sqrt(vgsr_err**2 + (10**lsigv_vals)**2))
    lstream_pmra = stats.norm.logpdf(pmra, loc=pmra_vals, scale=np.sqrt(pmra_err**2 + (10**lsigpmra)**2))
    lstream_pmdec = stats.norm.logpdf(pmdec, loc=pmdec_vals, scale=np.sqrt(pmdec_err**2 + (10**lsigpmdec)**2))
    ...
    # Background components (simplified - assume non-truncated for membership prob)
    lbg_v = stats.norm.logpdf(vgsr, loc=bv, scale=np.sqrt(vgsr_err**2 + (10**lsigbv)**2))
    lbg_pmra = stats.norm.logpdf(pmra, loc=bpmra, scale=np.sqrt(pmra_err**2 + (10**lsigbpmra)**2))
    lbg_pmdec = stats.norm.logpdf(pmdec, loc=bpmdec, scale=np.sqrt(pmdec_err**2 + (10**lsigbpmdec)**2))
    ...
    # Combine components
    lstream = np.log(pstream_vals) + lstream_v + lstream_feh + lstream_pmra + lstream_pmdec
    lbg = np.log(1-pstream_vals) + lbg_v + lbg_feh + lbg_pmra + lbg_pmdec
